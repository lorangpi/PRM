...
class RoboSuite_PickPlace_Detector:
    def __init__(self, env, single_object_mode=True, object_to_use='can', factor=1, return_int=False):
        ...
        self.types = {"location":"physobj", "object":"physobj", "gripper":"physobj"}  # Types of entities in the environment
        self.obj_types = {obj: "object" for obj in self.objects}
        self.obj_types.update({area: "location" for area in self.object_areas})
        self.obj_types.update({gripper: "gripper" for gripper in self.grippers})

        # Define a function that returns int if return_int is True else do nothing
    ...
    def at(self, obj, area, return_distance=False):
        ...
        if return_distance:
            return self.r_int(dist * self.factor)
        else:
    ...
    def picked_up(self, obj, return_distance=False):
        ...
        if return_distance:
            return self.r_int(z_dist * self.factor)
        else:
    ...
    def open(self, obj, return_distance=False):
        ...
        if obj == 'door':
            ...
            if self.env.door.lock:
                ...
                return int(relative_aperture) if return_distance else bool(relative_aperture > 10)
    ...
    def over(self, gripper, obj, return_distance=False):
        ...
        if gripper == 'gripper':
            ...
            if return_distance:
                return self.r_int(dist_xy * self.factor)
            else:
    ...
    def at_grab_level(self, gripper, obj, return_distance=False):
        ...
        if gripper == 'gripper':
            ...
            if return_distance:
                return self.r_int(dist_z * self.factor)
            else:
