...
class HERPlanWrapper(gym.Wrapper):
    ...
    def __init__(self, env, task_goal, actions, num_timesteps=None, detector=None, domain="domain_numerical", problem="problem_dummy", pddl_path="./PDDL_files/"):
        ...
        goal_ranges = self.detector.get_ranges(task_goal)
        # self.detector.get_ranges(task_goal.grounded_predicates) in the form a list of tuples [(min, max), ...]
        self.observation_space = gym.spaces.Dict({
            "observation": observation_space,
            "desired_goal": gym.spaces.Box(low=np.array([goal[0] for goal in goal_ranges]), high=np.array([goal[1] for goal in goal_ranges]), dtype=np.float32),
    ...
    def step(self, action):
        ...
        achieved_goal = np.asarray([v for k, v in sorted(state.grounded_predicates.items()) if k in self.task_goal.keys()])
        desired_goal = np.asarray([v for k, v in sorted(self.desired_goal.grounded_predicates.items())])
        observation = {
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        # Compute the reward for the current achieved goal and the desired goal (both are a batch of goals)
